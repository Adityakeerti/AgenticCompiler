...
## Appendix C — Dataset Generation Schema for RL Agent

To train the reinforcement learning policy, an error-correction dataset must be created based on the CPY syntax. The dataset should be generated in **CSV format** and contain the following columns:

| Column | Description |
|--------|-------------|
| `id` | Unique identifier for the sample |
| `original_code` | The CPY code containing the bug |
| `error_type` | Lexical, Syntactic, or Semantic |
| `error_message` | The exact error message outputted by the compiler |
| `error_line` | The line number where the error is detected |
| `target_action` | The intended fix operation (e.g., `INSERT_SEMICOLON`, `PREPEND_LET`, `DELETE_CHAR_AT_INDEX`) |
| `fixed_code` | The fully corrected CPY code |

### Example CSV Data

```csv
id,original_code,error_type,error_message,error_line,target_action,fixed_code
1,"let x = 10\nprint(x);",Syntactic,"Expected ';' after expression",1,INSERT_SEMICOLON,"let x = 10;\nprint(x);"
2,"let score = 90;\nprint(score$);",Lexical,"Unexpected character '$'",2,DELETE_CHAR_AT_INDEX,"let score = 90;\nprint(score);"
3,"x = 5;\nprint(x);",Semantic,"Variable 'x' used before declaration",1,PREPEND_LET,"let x = 5;\nprint(x);"
4,"let arr = [1, 2;\nprint(arr);",Syntactic,"Expected ']' after array elements",1,REPLACE_CHAR_AT_END,"let arr = [1, 2];\nprint(arr);"
5,"let y = 10;\nlet y = 20;",Semantic,"Variable 'y' already declared",2,REMOVE_LET_OR_RENAME,"let y = 10;\ny = 20;"
```
